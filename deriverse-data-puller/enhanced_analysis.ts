...
async function enhancedDeriverseAnalysis() {
  ...
  try {
    ...
    const analyzed: AccountAnalysis[] = allAccounts.map(acc => {
      ...
      else if (discriminator === '2000000001000000') accountType = 'Admin Account';
      else if (data.length === 248) accountType = 'Trader Account (size match)';
      else if (data.length === 376 || data.length === 408) accountType = 'Position Account (size match)';
      else if (data.length === 344 || data.length === 360) accountType = 'Order Account (size match)';
      else if (data.length === 64) accountType = 'Small Config Account';
      else if (data.length === 24) accountType = 'Fee/Utility Account';
    ...
    const byType = new Map<string, AccountAnalysis[]>();
